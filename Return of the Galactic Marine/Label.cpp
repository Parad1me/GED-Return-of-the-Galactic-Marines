...
Label::Label(const char * str, TTF_Font *font, SDL_Color colour, GLfloat x, GLfloat y, GLfloat scale){
	texID = textToTexture(str, font, colour, width, height);
	posX = x;
	posY = y;
	texScale = scale;
...
void Label::displayString(float x, float y, const char * str,TTF_Font * textFont)
{
    ...
    glBegin(GL_QUADS);
	  glTexCoord2d(0,0); // Texture has origin at top not bottom
      glVertex2f (x,y); // first corner
	  glTexCoord2d(1,0);
	  glVertex2f (x+1.0f*width, y); // second corner
	  glTexCoord2d(1,1);
      glVertex2f (x+1.0f*width, y+1.0f*height); // third corner
	  glTexCoord2d(0,1);
      glVertex2f (x, y+1.0f*height); // fourth corner
    glEnd();
...
void Label::setText(const char * str,TTF_Font * textFont)
{
	glDeleteTextures(1, &texID);
	texID = textToTexture(str, textFont, texColour, width, height);
}
